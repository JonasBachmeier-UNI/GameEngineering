...
const ID_PLACEHOLDER = 69

## Format: Zeit, Event, unit1, unit2


func get_time():
	return Time.get_ticks_msec()

func make_line(event, uid1, uid2):
	get_time()
	var line = str(get_time()) + "," + event + "," + str(uid1) + "," + str(uid2) + "\n"
...
func on_attack_selected(unit):
	var unit_id = unit.unit_id
	make_line("Attack Selected", unit_id, ID_PLACEHOLDER)

func on_wait_selected(unit):
	var unit_id = unit.unit_id
	make_line("Wait Selected", unit_id, ID_PLACEHOLDER)

func on_move_selected(unit):
	var unit_id = unit.unit_id
	make_line("Move Selected", unit_id, ID_PLACEHOLDER)
